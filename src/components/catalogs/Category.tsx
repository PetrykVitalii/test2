...
import { selectCategories, selectIsComming } from '@/store/selectors/categories';
...
const Category: React.FC<Props> = ({ match }) => {
  ...
  const handleSelectCategory = (obj: IСategory) => () => {
    dispatch(catalogsActions.setCategory(obj));

    goBack();
  };

  const goBack = () => history.push(`/catalogs/${match.params.id}/business-details`);
