...
import { emptyItemsActions, saveItemsToCatalog, sendItems } from '@/store/actions/emptyItems';
...
const AddItemsPage: React.FC = () => {
  const match = useParams<{catalogId: string}>();
  ...
  useEffect(() => {
    if (items.length > 1) {
      window.scrollTo({ top: wrapper.current.offsetHeight - 400, behavior: 'auto' });
    }
  }, [items.length]);
  ...
  const saveItems = () => {
    ...
    if (match && match.catalogId) {
      dispatch(saveItemsToCatalog(match.catalogId));
    } else {
      dispatch(sendItems());
    }
  };
  ...
  const addItemHandler = () => {
    dispatch(emptyItemsActions.addEmptyItem());
    window.scrollTo({ top: wrapper.current.offsetHeight - 350, behavior: 'smooth' });
  };
      ...
      <FixedHeader>
        <HeaderWrap>
          <HeaderSmallWrap>
            ...
            <AddItemsTitle>{itemsLan.add_items_title}</AddItemsTitle>
          </HeaderSmallWrap>
        </HeaderWrap>
      </FixedHeader>
      <View>
